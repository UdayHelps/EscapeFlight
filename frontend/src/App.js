...
function StatusBadge({ status }) {
  const styles = {
    ...
    DIVERTED:  { background:"#1e1040", color:"#a78bfa", border:"1px solid #4c1d95" },
  ...
  return (
    <span style={{ ...s, fontSize:10, padding:"2px 8px", borderRadius:4, fontWeight:"bold", letterSpacing:"1px", fontFamily:"inherit", whiteSpace:"nowrap" }}>
      {status}
    </span>
  );
}
...
const COORDS = {
  ...
  SCL:[-70.79,-33.39],ADD:[38.80,8.98],LOS:[3.32,6.58],ACC:[-0.17,5.61],
};
...
function RouteMapSVG({ path }) {
  ...
  return (
    <svg width={W} height={H} style={{width:"100%"}} viewBox={`0 0 ${W} ${H}`}>
      <defs>
        <marker id={`a${uid}`} markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto">
...
// ── Airport search input with live autocomplete ───────────────────────
        ...
        <div style={{ position:"absolute", top:"calc(100% + 4px)", left:0, right:0, background:"#1e293b",
          ...
          {suggestions.map((s) => (
            <div key={s.code} onClick={() => { onChange(s.code); setOpen(false); }}
              ...
              <span style={{ color:"#94a3b8", fontSize:11, textAlign:"right" }}>{s.city}, {s.country}</span>
            </div>
          ))}
        </div>
      )}
...
export default function App() {
  ...
  useEffect(() => {
    fetch(`${API}/api/health`).then((r) => r.json()).then(setApiHealth).catch(() => {});
  }, []);
    ...
    <div style={{ minHeight:"100vh", background:"#030712", color:"#e2e8f0" }}>
      ...
      <header style={{ borderBottom:"1px solid #1e293b", background:"#030712ee", backdropFilter:"blur(12px)", position:"sticky", top:0, zIndex:50 }}>
        <div style={{ maxWidth:1400, margin:"0 auto", padding:"12px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            <div>
              <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:18, letterSpacing:"-0.5px", color:"#f1f5f9" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:16, fontSize:11, flexWrap:"wrap" }}>
            {apiHealth && (
              <span style={{ color:"#4ade80", fontSize:10 }}>
                ✓ OpenSky · {apiHealth.airportsLoaded?.toLocaleString()} airports loaded
              </span>
            )}
      ...
      <div style={{ maxWidth:1400, margin:"0 auto", padding:"20px 16px" }}>
        ...
        <div style={{ background:"#0f172a", border:"1px solid #1e293b", borderRadius:16, padding:"20px 24px", marginBottom:24 }}>
          ...
          <div style={{ fontSize:11, color:"#475569", marginBottom:16 }}>Search any airport worldwide — type IATA code or city name</div>
          <div style={{ display:"flex", gap:12, flexWrap:"wrap", alignItems:"flex-end" }}>
            ...
            <button onClick={handleSearch} disabled={loading}
              style={{ background: loading?"#334155":"linear-gradient(135deg,#f97316,#ef4444)", border:"none", borderRadius:8, padding:"10px 28px", color:"#fff", fontSize:13, fontFamily:"JetBrains Mono, monospace", fontWeight:"bold", letterSpacing:"2px", cursor: loading?"not-allowed":"pointer", whiteSpace:"nowrap" }}>
              {loading ? "SCANNING..." : "FIND ROUTES ▸"}
            </button>
          </div>

          {error && (
            <div style={{ marginTop:12, color:"#f87171", fontSize:12, background:"#2d0f0f", padding:"8px 14px", borderRadius:6, border:"1px solid #7f1d1d" }}>
              ⚠ {error}
            </div>
          )}

          {result?.errors?.length > 0 && (
            <div style={{ marginTop:8, fontSize:10, color:"#64748b" }}>Notices: {result.errors.join(" | ")}</div>
          )}
        ...
        {loading && (
          <div style={{ textAlign:"center", padding:"80px 20px" }}>
            <div style={{ fontSize:11, color:"#64748b", letterSpacing:"4px", marginBottom:20 }}>FETCHING LIVE OPENSKY DATA<span className="blink">_</span></div>
            ...
            <div style={{ marginTop:20, fontSize:11, color:"#334155" }}>Querying OpenSky Network for last 24h of real flights...</div>
          </div>
        )}
          ...
          <div className="fade-in">
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16, flexWrap:"wrap", gap:8 }}>
              <div style={{ fontSize:11, color:"#64748b" }}>
                {result.origin?.city} ({result.origin?.iata}) → {result.destination?.city} ({result.destination?.iata}) · Last 24h real flight data
              </div>
              <span style={{ fontSize:10, color:"#4ade80", border:"1px solid #4ade8044", borderRadius:4, padding:"2px 8px" }}>
                ◉ OPENSKY NETWORK LIVE
              </span>
            </div>

            <div className="main-grid" style={{ display:"grid", gridTemplateColumns:"clamp(280px,30%,360px) 1fr", gap:20 }}>

              {/* LEFT: Routes panel */}
              <div>
                <div style={{ fontSize:10, color:"#64748b", letterSpacing:"3px", marginBottom:12 }}>▸ ESCAPE ROUTES RANKED</div>
                {result.routes.length === 0 ? (
                  <div style={{ background:"#0f172a", border:"1px solid #1e293b", borderRadius:12, padding:24, textAlign:"center", color:"#64748b", fontSize:12 }}>
                    No flight data for this route in the last 24h.<br/>Try a busier route or major hub pair.
                  </div>
                ) : (
                  result.routes.map((route, idx) => (
                    <div key={route.id} className="route-card"
                      ...
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                        <div>
                          <div style={{ fontSize:10, color:"#64748b", marginBottom:4 }}>
                            {idx===0?"🟢 BEST ROUTE":idx===1?"🟡 ALTERNATE":"🔴 FALLBACK"}
                          </div>
                      ...
                      <div style={{ display:"flex", gap:12, marginTop:10, fontSize:11, color:"#64748b", flexWrap:"wrap" }}>
                        ...
                        <span><span style={{color:"#94a3b8"}}>◷{route.scheduled}</span> sched.</span>
                      </div>
                    </div>
                  ))
                )}
                ...
                <div style={{ background:"#7c2d12", border:"1px solid #9a3412", borderRadius:10, padding:"12px 14px", marginTop:8, fontSize:11 }}>
                  ...
                  <div style={{ color:"#fdba74", lineHeight:1.5 }}>
                    Verify active NOTAMs and airspace restrictions before travel. Data reflects last 24h of actual operations.
                  </div>
                </div>
              </div>
                    ...
                    <div className="fade-in" style={{ background:"#0f172a", border:"1px solid #1e293b", borderRadius:12, overflow:"hidden" }}>
                      ...
                      {selectedRoute.flights.length === 0 ? (
                        <div style={{ padding:40, textAlign:"center", color:"#64748b", fontSize:12 }}>
                          No flights found for this route in the last 24h.
                        </div>
                      ) : (
                            ...
                            <div key={a.name} style={{background:"#0f172a",border:"1px solid #1e293b",borderRadius:12,padding:"16px 20px"}}>
                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                                <div>
                                  ...
                                  <div style={{fontSize:11,color:"#64748b",marginTop:2}}>{a.landed} landed · {a.inAir||0} in air · {a.cancelled} cancelled · {a.total} total</div>
                                </div>
                                <div style={{display:"flex",alignItems:"center",gap:12}}>
                                  {i===0 && <span style={{fontSize:10,color:"#facc15"}}>★ TOP CARRIER</span>}
                  ...
                  {activeTab==="predict" && (
                    <div className="fade-in">
                      ...
                      {upcomingFlights.length===0 ? (
                        <div style={{background:"#0f172a",border:"1px solid #1e293b",borderRadius:12,padding:40,textAlign:"center",color:"#64748b",fontSize:12}}>
                          No upcoming or in-air flights in current data window.
                        </div>
                      ) : (
                      ...
                      <div style={{marginTop:16,background:"#0f172a",border:"1px dashed #334155",borderRadius:10,padding:"14px 18px",fontSize:11,color:"#64748b",lineHeight:1.8}}>
                        <div style={{color:"#475569",marginBottom:6,fontSize:10,letterSpacing:"2px"}}>◈ PREDICTION MODEL</div>
                        Airline reliability from real OpenSky data (55%) · Route success rate (35%) · Baseline (10%)
                        <br/>Data: Last 24h · Source: OpenSky Network
                      </div>
                    </div>
                  )}
                </div>
        ...
        {!loading && !result && (
          <div style={{textAlign:"center",padding:"80px 20px",color:"#475569"}}>
            ...
            <div style={{fontSize:11,marginTop:8}}>
              Powered by OpenSky Network · 6,000+ airports worldwide · Real ADS-B transponder data
            </div>
          </div>
        )}
        ...
        <div style={{marginTop:40,padding:"20px 0",borderTop:"1px solid #1e293b",display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8,fontSize:10,color:"#334155"}}>
          ...
          <span>POWERED BY: OpenSky Network (opensky-network.org)</span>
          <span>FOR INFORMATIONAL USE ONLY — VERIFY WITH AIRLINES</span>
        </div>
      </div>
    </div>
  );
